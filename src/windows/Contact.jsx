      ...
      <div className="p-4 sm:p-8 space-y-4 sm:space-y-6 flex-1 overflow-y-auto">
        ...
        <ul className="grid grid-cols-2 gap-2 sm:gap-3">
          {socials.map(({ id, bg, link, icon, text }) => (
            <li
              key={id}
              className="rounded-lg sm:rounded-xl p-2 sm:p-4 hover:-translate-y-1 hover:scale-[1.02] transition-all duration-300 shadow-md hover:shadow-xl"
              ...
              >
                <div className="w-7 h-7 sm:w-10 sm:h-10 rounded-full bg-white/20 flex items-center justify-center backdrop-blur-sm">
                  <img
                    src={icon}
                    alt={text}
                    className="w-3 h-3 sm:w-5 sm:h-5 brightness-0 invert"
                  />
                </div>
                <div className="flex flex-col">
